...
namespace Ffxi_Navmesh_Builder.Common.dat.Types
{
    ...
    public class Mzb
    {
        ...
        internal void ParseMzb(Span<byte> block)
        {
            try
            {
                // Convert the Span<byte> to a hexadecimal string
                StringBuilder hexString = new StringBuilder(block.Length * 3); // Each byte is 2 chars plus a space
                for (int i = 0; i < block.Length; i++)
                {
                    hexString.Append(block[i].ToString("X2"));
                    hexString.Append(" "); // Add a space between bytes
                    if ((i + 1) % 16 == 0) // Optional: Line break every 16 bytes for readability
                    {
                ...
                if (gridOffset > 0)
                {
                    ...
                    var gridwidth = block[0x0c];
                    var gridheight = block[0x0d];
